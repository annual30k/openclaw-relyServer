...
CREATE TABLE IF NOT EXISTS file_transfers (
  ...
  expires_at DATETIME NOT NULL,
  sort_timestamp_ms BIGINT UNSIGNED NOT NULL DEFAULT 0,
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (file_id),
  UNIQUE KEY uk_file_transfers_upload_id (upload_id),
  KEY idx_file_transfers_gateway_session (gateway_id, session_key),
  KEY idx_file_transfers_gateway_status_sort_timestamp_ms (gateway_id, status, sort_timestamp_ms),
  KEY idx_file_transfers_gateway_session_status_sort_timestamp_ms (gateway_id, session_key, status, sort_timestamp_ms),
